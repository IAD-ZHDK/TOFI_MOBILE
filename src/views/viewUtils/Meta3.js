...
void main() {
   ...
   // vec2 coord = vPos.xy/dim.xy;
    ...
    float m_dist = 1.9;  // minimum distance

    // Iterate through the points positions
    for (int i = 0; i < ${ballCount}; i++) {
        vec2 vectLength = balls[i].xy-st;
...
class Meta {
  ...
  update(Tone) {
    ...
    for (const ball of this.balls) {
      ballsUniformArray.push((ball.x/this.width))
      ballsUniformArray.push((this.height-(ball.y)/this.height))
